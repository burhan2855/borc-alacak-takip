...
class SalaryCalculator {
    fun calculateRangeSalary(
        ...
        val totalDaysInRange = startDate.daysUntil(endDate) + 1
        val dailyValue = monthlyBaseline / totalDaysInRange.toDouble()
        val overtimeHourlyRate = if (employee.dailyWorkingHours > 0) monthlyBaseline / (totalDaysInRange.toDouble() * employee.dailyWorkingHours) else 0.0
        
        var effectiveMissingEquivDays = 0.0
        var currentDate = startDate
        while (currentDate <= endDate) {
            val dayLogs = logs.filter { it.date == currentDate }
            val isBeforeStart = employee.startDate?.let { currentDate < it } ?: false
            val isAfterEnd = employee.endDate?.let { currentDate > it } ?: false
            ...
            if (isBeforeStart || isAfterEnd) {
                ...
                if (hasSick) {
                    ...
                    if (workLog != null) {
                        workingDays++
                        val hoursWorked = (workLog.endTime!!.toInstant(TimeZone.UTC).epochSeconds - workLog.startTime!!.toInstant(TimeZone.UTC).epochSeconds - workLog.breakDurationMinutes * 60).toDouble() / 3600.0
        ...
        val adjSummary = calculateAdjustmentSummary(adjustments.filter { it.date >= startDate && it.date <= endDate })
        val baseDaysForCalc = totalDaysInRange.toDouble()
    ...
    fun calculateIndemnity(employee: Employee, allLogs: List<WorkLog>, allAdjustments: List<Adjustment>): IndemnityReport? {
        ...
        val totalDaysWorked = start.daysUntil(end) + 1
        var y = 0; var m = 0; var d = 0
        var temp = start
        while (temp.plus(1, DateTimeUnit.YEAR) <= end) { y++; temp = temp.plus(1, DateTimeUnit.YEAR) }
        while (temp.plus(1, DateTimeUnit.MONTH) <= end) { m++; temp = temp.plus(1, DateTimeUnit.MONTH) }
        d = temp.daysUntil(end) + 1
        ...
        val kidem = (monthlySalary / 365.0) * totalDaysWorked.toDouble()

        val dailyMaas = monthlySalary / 30.0
        val adjSum = calculateAdjustmentSummary(allAdjustments)
        val logsSum = allLogs.filter { it.date >= start && it.date <= end }.let { periodLogs ->
